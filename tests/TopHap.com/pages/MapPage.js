...
import { bottomMenuBtnsWithHoverOversTxt, bottomMenuBtns } from '../fixtures/assert';
...
class MapPage extends BasePage {
  ...
  moveToMenuBtn(text) {
    helper.moveToElement(this.menuBtn(text));
  }
  ...
  submitSearch(searchCriteria){
    ...
    browser.pause(2000);
    helper.clickElement(this.searchBtn);
    this.waitForResultList();
  }

  waitForResultList(){
    helper.elementsAreLoaded(this.searchItems);
    browser.waitUntil( () => +this.propertiesCount.getText().split(' ')[0] !== 0);
  }

  openFilterMenu(filterType){
    if (!filterType.isDisplayed()){
      helper.clickElement(this.moreFiltersBtn);
    }
    helper.clickElement(filterType);
  }
  ...
  applyPropertyStatusFilter(filterType, filterOption){
    ...
    helper.clickElement(filterOption);
    this.waitForResultList();
  }
  ...
  applySort(orderAtoZorZtoA, sortOption){
    helper.clickElement(this.sortMenu);
    helper.clickElement(orderAtoZorZtoA);
    helper.clickElement(sortOption);
  }
  ...
  clearOldSearchAndFilterRecords() {
    if (this.clearSearchFilterBtns.length > 0) {
      for (let clearSearchFilterBtn of this.clearSearchFilterBtns) {
        helper.clickElement(clearSearchFilterBtn);
      }
    }
  }
  ...
  getResultsList(locator) {
    ...
    while (currentSearchResultList.length > 0) {
      ...
      helper.movingIsFinished(currentElement);
